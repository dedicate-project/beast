...
int main(int /*argc*/, char** /*argv*/) {
  ...
  /* Here, the actual program is declared. It consists of only two operators:
     Setting the above
     "Hello World!" message string in the string table index, and printing from
     that index. */
  beast::Program prg;
  prg.setStringTableEntry(string_table_index, hello_world_message);
  prg.printStringFromStringTable(string_table_index);

  /* A VmSession instance is required for execution. This one has no variable
     space, and can hold exactly one string table item with maximum length of 12
     characters, matching the string above. */
  ...
  /* Here, the program is executed through its session step by step. In each
     step, the print buffer is read and printed, and the internal session print
     buffer variable is cleared. This is important, as otherwise the VmSession
     may throw an exception due to an overflow. */
