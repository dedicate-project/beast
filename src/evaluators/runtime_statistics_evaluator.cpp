...
double RuntimeStatisticsEvaluator::evaluate(const VmSession& session) {
  ...
  /*
    ...
    structure AND dynamic runtime behavior, three things must be met:

    * As few operators executed as possible should be noop operators. This means
    that we don't waste time on empty cycles but execute efficiently. The
    variable holding a measure of this is steps_executed_noop_fraction.

    * As many operators actually present in the program code as possible should
    be noop operators. This means that the program's function can be executed
    with a slender static structure rather than bloated code. The variable
    holding a measure of this is total_steps_noop_fraction.

    * As few individual operators as possible should actually be executed. This
    means that a program makes good use of iterations and jumps, and doesn't
    rely on overfitting to any particular task. The variable holding a measure
    of this is program_executed_fraction.

    These aspects need to be combined into a single score value. Given the
    considerations from above, the formula for this score value hence becomes:
     ...
     The weight values dyn_noop_weight and stat_noop_weight must be chosen
    carefully so that programs can correctly converge to a good balance between
