...
bool CpuVirtualMachine::step(VmSession& session, bool dry_run) {
  ...
  case OpCode::SetVariable: {
    ...
    debug("set_variable(" + std::to_string(variable_index) + ", " + std::to_string(variable_content) + ", " +
  ...
  case OpCode::AddConstantToVariable: {
    ...
    debug("add_constant_to_variable(" + std::to_string(variable_index) + ", " + to_string(follow_links) + ", " +
  ...
  case OpCode::AddVariableToVariable: {
    ...
    debug("add_variable_to_variable(" + std::to_string(source_variable) + ", " + to_string(follow_source_links) + ", " +
  ...
  case OpCode::SubtractConstantFromVariable: {
    ...
    const bool follow_links = session.getData1() != 0x0;
    const int32_t constant = session.getData4();
    debug("subtract_constant_from_variable(" + std::to_string(variable_index) + ", " + to_string(follow_links) + ", " +
          std::to_string(constant) + ")");
    if (!dry_run) {
  ...
  case OpCode::SubtractVariableFromVariable: {
    ...
    const int32_t destination_variable = session.getData4();
    const bool follow_destination_links = session.getData1() != 0x0;
    debug("subtract_variable_from_variable(" + std::to_string(source_variable) + ", " + to_string(follow_source_links) +
          ", " + std::to_string(destination_variable) + ", " + to_string(follow_destination_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::RelativeJumpToVariableAddressIfVariableGt0: {
    ...
    debug("relative_jump_to_variable_address_if_variable_gt_0(" + std::to_string(condition_variable) + ", " +
          to_string(follow_condition_links) + ", " + std::to_string(addr_variable) + ", " +
          to_string(follow_addr_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::LoadMemorySizeIntoVariable: {
    ...
    const bool follow_links = session.getData1() != 0x0;
    debug("load_memory_size_into_variable(" + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::CheckIfVariableIsInput: {
    ...
    const int32_t destination_variable = session.getData4();
    const bool follow_destination_links = session.getData1() != 0x0;
    debug("check_if_variable_is_input(" + std::to_string(source_variable) + ", " + to_string(follow_source_links) +
          ", " + std::to_string(destination_variable) + ", " + to_string(follow_destination_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::CheckIfVariableIsOutput: {
    ...
    const int32_t destination_variable = session.getData4();
    const bool follow_destination_links = session.getData1() != 0x0;
    debug("check_if_variable_is_output(" + std::to_string(source_variable) + ", " + to_string(follow_source_links) +
          ", " + std::to_string(destination_variable) + ", " + to_string(follow_destination_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::LoadInputCountIntoVariable: {
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("load_input_count_into_variable(" + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::LoadOutputCountIntoVariable: {
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("load_output_count_into_variable(" + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::LoadCurrentAddressIntoVariable: {
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("load_current_address_into_variable(" + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::SetStringTableEntry: {
    ...
    debug("set_string_table_entry(" + std::to_string(string_table_index) + ", " + std::to_string(string_length) +
          ", '" + string_content + "')");
    if (!dry_run) {
  ...
  case OpCode::LoadStringTableLimitIntoVariable: {
    const int32_t variable_index = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("load_string_table_limit_into_variable(" + std::to_string(variable_index) + ", " + to_string(follow_links) +
          ")");
    if (!dry_run) {
  ...
  case OpCode::CopyVariable: {
    ...
    const int32_t destination_variable = session.getData4();
    const bool follow_destination_links = session.getData1() != 0x0;
    debug("copy_variable(" + std::to_string(source_variable) + ", " + to_string(follow_source_links) + ", " +
          std::to_string(destination_variable) + ", " + to_string(follow_destination_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::BitShiftVariableLeft: {
    ...
    debug("bit_shift_variable_left(" + std::to_string(variable_index) + ", " + to_string(follow_links) + ", " +
  ...
  case OpCode::BitShiftVariableRight: {
    ...
    debug("bit_shift_variable_right(" + std::to_string(variable_index) + ", " + to_string(follow_links) + ", " +
          std::to_string(places) + ")");
    if (!dry_run) {
      session.bitShiftVariable(variable_index, follow_links, places);
    }
  ...
  case OpCode::BitWiseInvertVariable: {
    const int32_t variable_index = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("bit_wise_invert_variable(" + std::to_string(variable_index) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::BitWiseAndTwoVariables: {
    ...
    debug("bit_wise_and_two_variables(" + std::to_string(variable_index_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_index_b) + ", " + to_string(follow_links_b) + ")");
    if (!dry_run) {
  ...
  case OpCode::BitWiseOrTwoVariables: {
    ...
    const int32_t variable_index_b = session.getData4();
    const bool follow_links_b = session.getData1() != 0x0;
    debug("bit_wise_or_two_variables(" + std::to_string(variable_index_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_index_b) + ", " + to_string(follow_links_b) + ")");
    if (!dry_run) {
  ...
  case OpCode::BitWiseXorTwoVariables: {
    ...
    const int32_t variable_index_b = session.getData4();
    const bool follow_links_b = session.getData1() != 0x0;
    debug("bit_wise_xor_two_variables(" + std::to_string(variable_index_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_index_b) + ", " + to_string(follow_links_b) + ")");
    if (!dry_run) {
  ...
  case OpCode::LoadRandomValueIntoVariable: {
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("load_random_value_into_variable(" + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::ModuloVariableByConstant: {
    ...
    debug("modulo_variable_by_constant(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
  ...
  case OpCode::ModuloVariableByVariable: {
    ...
    debug("modulo_variable_by_variable(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(modulo_variable) + ", " + to_string(modulo_follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::PushVariableOnStack: {
    ...
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("push_variable_on_stack(" + std::to_string(stack_variable) + ", " + to_string(stack_follow_links) + ", " +
          std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::PushConstantOnStack: {
    ...
    const bool stack_follow_links = session.getData1() != 0x0;
    const int32_t constant = session.getData4();
    debug("push_constant_on_stack(" + std::to_string(stack_variable) + ", " + to_string(stack_follow_links) + ", " +
          std::to_string(constant) + ")");
    if (!dry_run) {
  ...
  case OpCode::PopVariableFromStack: {
    ...
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("pop_variable_from_stack(" + std::to_string(stack_variable) + ", " + to_string(stack_follow_links) + ", " +
          std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::PopTopItemFromStack: {
    const int32_t stack_variable = session.getData4();
    const bool stack_follow_links = session.getData1() != 0x0;
    debug("pop_top_item_from_stack(" + std::to_string(stack_variable) + ", " + to_string(stack_follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::CheckIfStackIsEmpty: {
    ...
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("check_if_stack_is_empty(" + std::to_string(stack_variable) + ", " + to_string(stack_follow_links) + ", " +
          std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::PrintVariableStringFromStringTable: {
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("print_variable_string_from_string_table(" + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::LoadVariableStringItemLengthIntoVariable: {
    ...
    debug("load_variable_string_item_length_into_variable(" + std::to_string(string_item_variable) + ", " +
          to_string(string_item_follow_links) + ", " + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::TerminateWithVariableReturnCode: {
    const int32_t variable = session.getData4();
    const bool follow_links = session.getData1() != 0x0;
    debug("terminate_with_variable_return_code(" + std::to_string(variable) + ", " + to_string(follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::VariableBitShiftVariableLeft: {
    ...
    debug("variable_bit_shift_variable_left(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(places_variable) + ", " + to_string(places_follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::VariableBitShiftVariableRight: {
    ...
    const int32_t places_variable = session.getData4();
    const bool places_follow_links = session.getData1() != 0x0;
    debug("variable_bit_shift_variable_right(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(places_variable) + ", " + to_string(places_follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::VariableRotateVariableLeft: {
    ...
    const int32_t places_variable = session.getData4();
    const bool places_follow_links = session.getData1() != 0x0;
    debug("variable_rotate_variable_left(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(places_variable) + ", " + to_string(places_follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::VariableRotateVariableRight: {
    ...
    const int32_t places_variable = session.getData4();
    const bool places_follow_links = session.getData1() != 0x0;
    debug("variable_rotate_variable_right(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(places_variable) + ", " + to_string(places_follow_links) + ")");
    if (!dry_run) {
  ...
  case OpCode::CompareIfVariableGtConstant: {
    ...
    debug("compare_if_variable_gt_constant(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(constant) + ", " + std::to_string(target_variable) + ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::CompareIfVariableLtConstant: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("compare_if_variable_lt_constant(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(constant) + ", " + std::to_string(target_variable) + ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::CompareIfVariableEqConstant: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("compare_if_variable_eq_constant(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(constant) + ", " + std::to_string(target_variable) + ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::CompareIfVariableGtVariable: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("compare_if_variable_gt_variable(" + std::to_string(variable_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_b) + ", " + to_string(follow_links_b) + ", " + std::to_string(target_variable) +
          ", " + to_string(target_follow_links));
    if (!dry_run) {
      session.compareIfVariableGtVariable(
          variable_a, follow_links_a, variable_b, follow_links_b, target_variable, target_follow_links);
    }
  } break;
  ...
  case OpCode::CompareIfVariableLtVariable: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("compare_if_variable_lt_variable(" + std::to_string(variable_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_b) + ", " + to_string(follow_links_b) + ", " + std::to_string(target_variable) +
          ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::CompareIfVariableEqVariable: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("compare_if_variable_eq_variable(" + std::to_string(variable_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_b) + ", " + to_string(follow_links_b) + ", " + std::to_string(target_variable) +
          ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::GetMaxOfVariableAndConstant: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("get_max_of_variable_and_constant(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(constant) + ", " + std::to_string(target_variable) + ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::GetMinOfVariableAndConstant: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("get_min_of_variable_and_constant(" + std::to_string(variable) + ", " + to_string(follow_links) + ", " +
          std::to_string(constant) + ", " + std::to_string(target_variable) + ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::GetMaxOfVariableAndVariable: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("get_max_of_variable_and_variable(" + std::to_string(variable_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_b) + ", " + to_string(follow_links_b) + ", " + std::to_string(target_variable) +
          ", " + to_string(target_follow_links));
    if (!dry_run) {
  ...
  case OpCode::GetMinOfVariableAndVariable: {
    ...
    const int32_t target_variable = session.getData4();
    const bool target_follow_links = session.getData1() != 0x0;
    debug("get_min_of_variable_and_variable(" + std::to_string(variable_a) + ", " + to_string(follow_links_a) + ", " +
          std::to_string(variable_b) + ", " + to_string(follow_links_b) + ", " + std::to_string(target_variable) +
          ", " + to_string(target_follow_links));
    if (!dry_run) {
