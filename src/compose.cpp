 ...
 */
void serveStaticFile(crow::response* res, const std::string_view html_root, const std::string_view path) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    CROW_ROUTE(app, "/api/v1/pipelines/new").methods("POST"_method)([&pipeline_server](const crow::request& req) {
      return pipeline_server.serveNewPipeline(req);
    });
    CROW_ROUTE(app, "/api/v1/pipelines/<int>")
        .methods("GET"_method)([&pipeline_server](const crow::request& /*req*/, int32_t pipeline_id) {
          return pipeline_server.servePipelineById(static_cast<uint32_t>(pipeline_id));
        });
    CROW_ROUTE(app, "/api/v1/pipelines/<int>/<path>")
        .methods("GET"_method, "POST"_method)(
            [&pipeline_server](const crow::request& req, int32_t pipeline_id, const std::string_view path) {
              return pipeline_server.servePipelineAction(req, static_cast<uint32_t>(pipeline_id), path);
            });
    CROW_ROUTE(app, "/api/v1/pipelines").methods("GET"_method)([&pipeline_server](const crow::request& /*req*/) {
      return pipeline_server.serveAllPipelines();
    });
    CROW_ROUTE(app, "/<path>")
        .methods("GET"_method)([&html_root](const crow::request& /*req*/,
                                            crow::response& res,
                                            const std::string_view path) { serveStaticFile(&res, html_root, path); });
    CROW_ROUTE(app, "/").methods("GET"_method)([&html_root](const crow::request& /*req*/, crow::response& res) {
