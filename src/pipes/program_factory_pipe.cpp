...
ProgramFactoryPipe::ProgramFactoryPipe(uint32_t max_candidates, uint32_t max_size, uint32_t memory_size,
                                       uint32_t string_table_size, uint32_t string_table_item_length,
                                       std::shared_ptr<ProgramFactoryBase> factory)
    : Pipe(max_candidates), max_size_{max_size}, memory_size_{memory_size}, string_table_size_{string_table_size},
...
void ProgramFactoryPipe::execute() {
  while (output_.size() < max_candidates_) {
    ...
    Program program = factory_->generate(max_size_, memory_size_, string_table_size_, string_table_item_length_);
