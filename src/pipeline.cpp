...
void Pipeline::connectPipes(const std::shared_ptr<Pipe>& source_pipe, uint32_t source_slot_index,
  ...
  for (const Connection& connection : connections_) {
    if (connection.source_pipe == source_pipe && connection.source_slot_index == source_slot_index) {
...
void Pipeline::pipelineWorker(ManagedPipe& managed_pipe) {
  ...
  while (managed_pipe.should_run) {
    ...
    for (uint32_t slot_index = 0; slot_index < managed_pipe.pipe->getOutputSlotCount(); ++slot_index) {
      ...
      auto destination_slot_connection_iter = std::find_if(
          destination_connections.begin(), destination_connections.end(), [slot_index](const Connection* connection) {
      ...
      while (managed_pipe.pipe->hasOutput(slot_index) &&
             (destination_slot_connection->buffer.size() < destination_slot_connection->buffer.capacity())) {
