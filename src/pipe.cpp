...
Pipe::Pipe(uint32_t max_candidates) : max_candidates_{max_candidates} {}

void Pipe::addInput(const std::vector<unsigned char>& candidate) { input_.push_back(candidate); }

bool Pipe::hasSpace() const { return input_.size() < max_candidates_; }
...
bool Pipe::hasOutput(uint32_t /*slot_index*/) const { return !output_.empty(); }
